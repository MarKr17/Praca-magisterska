...
class Button():
    ...
    def draw(self):
        ...
        image = pygame.image.load(os.path.join(assets_path, "pause-button.png"))
        image = pygame.transform.scale(image, (self.width, self.height))
        #self.screen[0].blit(self.surf,(400,900))
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if button.x[0] <= mouse[0] <= button.x[1] and button.y[0] <= mouse[1] <= button.y[1]:
                    state= PAUSE
        if state == RUNNING:
